...
class LMDB(Dataset):
    def __init__(
        ...
        self.threed_5_points = config.get("threed_5_points")
        self.threed_68_points = config.get("threed_68_points")
    ...
    def __getitem__(self, index):
        ...
        projected_bbox_labels = bbox_labels
        pose_labels = []
        img = np.array(img)
        if False: # Don't handle pose for now
            # get pose labels
        ...
        target = {
            ...
            "event": torch.tensor([event_class]),
        }
        return img, target
    ...
    def __init__(self, config, lmdb_path, train=True):
        ...
        if train:
            if self.config.get("resize"):
                augmentation_methods.append(augmentation.resize)

            if self.config.get("random_flip"):
            ...
            if self.config.get("noise_augmentation"):
                augmentation_methods.append(augmentation.add_noise)

            if self.config.get("contrast_augmentation"):
                augmentation_methods.append(augmentation.change_contrast)

        if self.config.get("pose_mean") is not None:
