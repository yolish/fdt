...
import PIL
import json
from os.path import join, exists, split, realpath, dirname
import time
...
def init_logger(outpath=None):
    ...
    with open(join(path, 'log_config.json')) as json_file:
        ...
        if outpath is not None:
            create_output_dir(dirname(outpath))
            log_config_dict.get('handlers').get('file_handler')['filename'] = outpath
        else:
