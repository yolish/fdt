"""
Code for the position encoding of PDT
...
class PositionEmbeddingLearnedWithGlobalToken(nn.Module):
    """
    ...
    def __init__(self, num_pos_feats=256):
        ...
        self.pose_token_embed = nn.Embedding(100, num_pos_feats)
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        g = i[0]-1 # global token
        x_emb = self.col_embed(i)
        y_emb = self.row_embed(j)

        g_emb = torch.cat([self.pose_token_embed(g),self.pose_token_embed(g)]).repeat(x.shape[0], 1)
...
def build_position_encoding(config):
    ...
    if config.get("learn_with_global_token"):
        position_embedding = PositionEmbeddingLearnedWithGlobalToken(N_steps)
    else:
